...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const sksuma = x1 * x2 + y1 * y2;
  const xvector = Math.sqrt(x1 ** 2 + x2 ** 2);
  const yvector = Math.sqrt(y1 ** 2 + y2 ** 2);
  const cosT = sksuma / (xvector * yvector);
  const resultangle = Math.max(-1, Math.min(1, cosT));
  return Math.acos(resultangle);
}
...
function roundToPowerOfTen(num, pow) {
  const stepen = 10 ** pow;
  return Math.round(num / stepen) * stepen;
}
...
function toFixed(number, fractionDigits) {
  const fixnumber = fractionDigits;
