...
function getRectangleArea(width, height) {
  const resultat = parseInt(width) * parseInt(height);
  return resultat;
}
...
function getCircleCircumference(radius) {
  const resultat = 2 * Math.PI * parseInt(radius);
  return resultat;
}
...
function getAverage(value1, value2) {
  const resultat = (parseInt(value1) + parseInt(value2)) / 2;
  return resultat;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  let a = x1 * x2 + y1 * y2;
  let b = Math.sqrt(x1 ** 2 + y1 ** 2);
  let c = Math.sqrt(x2 ** 2 + y2 ** 2);
  return (resultat = Math.acos(a / (b * c)));
}
...
function roundToPowerOfTen(/* num, pow */) {
  const resultat = Math.round(num / 10 ** pow) * 10 ** pow;
  return resultat;
}
